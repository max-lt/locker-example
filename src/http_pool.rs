...
impl HttpPool {
    ...
    fn fetch(&self, worker_id: usize, step: usize) -> mpsc::Receiver<HttpResult> {
        ...
        let _ = self.tx.send(HttpRequest {
            worker_id,
            step,
            response_tx: tx,
        });
...
impl IsolatePool {
    ...
    fn release(&self, iso: PooledIsolate) {
        self.isolates.lock().unwrap().push(iso);
    }
}
...
impl V8Pool {
    ...
    fn execute(
        ...
        step: usize,
        fetch_data: Option<String>,
    ) -> mpsc::Receiver<V8Result> {
        let (tx, rx) = mpsc::channel();
        let _ = self.tx.send(V8Request {
            worker_id,
            step,
            fetch_data,
            response_tx: tx,
...
fn main() {
    ...
    println!(
        "Config: {} workers, {} isolates, {} steps/worker\n",
