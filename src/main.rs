                        ...
                        match Pin::new(rx).poll(cx) {
                            ...
                            Poll::Ready(result) => {
                                ...
                                if this.task.step >= this.task.total_steps {
                                    ...
                                    return Poll::Ready((
                                        this.task.worker_id,
                                        this.last_js_result.clone(),
                                    ));
                                } else {
                                    // Start fetch
                                    log(
                                        this.task.worker_id,
                                        &format!("Starting fetch for step {}...", this.task.step),
                                    );
                                    let url = format!(
                                        "https://api.mock/worker/{}/step/{}",
                                        this.task.worker_id, this.task.step
                                    );
                                    let rx = this.http_pool.fetch(
                                        this.task.worker_id,
                                        this.task.step,
                                        url,
                                    );
                        ...
                        match Pin::new(rx).poll(cx) {
                            ...
                            Poll::Ready(result) => {
                                ...
                                log(
                                    this.task.worker_id,
                                    &format!(
                                        "Fetch done from {}, submitting next JS...",
                                        fetch_result.thread_name
                                    ),
                                );
...
async fn main() {
    ...
    println!(
        "  - V8 Isolate Pool: {} isolates (shared across threads)",
        args.isolates
    );
    ...
    // Create thread pools
    let v8_pool = Arc::new(V8ThreadPool::new(
        args.v8_threads,
        Arc::clone(&isolate_pool),
    ));
