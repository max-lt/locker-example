...
fn now() -> String {
  ...
  let (h, m, s, ms) = ((d.as_secs() / 3600) % 24, (d.as_secs() / 60) % 60, d.as_secs() % 60, d.subsec_millis());
...
impl IsolatePool {
  fn new(num: usize) -> Self {
    let isolates = (0..num)
      ...
      .collect();
    Self { isolates: Mutex::new(isolates) }
  }
...
fn main() {
  let num_workers: usize = std::env::args().nth(1).and_then(|s| s.parse().ok()).unwrap_or(20);
  let num_isolates: usize = std::env::args().nth(2).and_then(|s| s.parse().ok()).unwrap_or(4);
  let num_threads: usize = std::env::args().nth(3).and_then(|s| s.parse().ok()).unwrap_or(2);
  let steps_per_worker: usize = std::env::args().nth(4).and_then(|s| s.parse().ok()).unwrap_or(3);
  ...
  println!("Config: {} workers, {} isolates, {} threads, {} steps/worker", num_workers, num_isolates, num_threads, steps_per_worker);
        ...
        match req {
          Ok(req) => {
            ...
            let result = {
              ...
              let code = format!("`w${{{}}} s${{{}}} = ${{{} * 100 + {}}}`", req.worker_id, req.step, req.worker_id, req.step);
  ...
  while let Ok(r) = result_rx.recv() {
    ...
    if count % 10 == 0 || count == total {
      println!("[{}] {}/{} - {} (t={} i={})", now(), count, total, r.result, r.thread_name, r.isolate_id);
    }
  }

  let elapsed = start.elapsed();
  println!("\n=== Completed {} ops in {:.2}s ({:.0} ops/sec) ===", total, elapsed.as_secs_f64(), total as f64 / elapsed.as_secs_f64());
}
